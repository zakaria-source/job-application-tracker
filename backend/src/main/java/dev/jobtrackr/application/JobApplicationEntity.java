...
public class JobApplicationEntity {
    ...
    @Column(name = "salary_target", precision = 14, scale = 2)
    private BigDecimal salaryTarget;

    @Enumerated(EnumType.STRING)
    @Column(name = "salary_period", nullable = false, length = 40)
    private SalaryPeriod salaryPeriod;

    @Column(name = "follow_up_date")
    private LocalDate followUpDate;

    @Column(name = "recruiter_name", length = 180)
    private String recruiterName;

    @Column(name = "recruiter_email", length = 320)
    private String recruiterEmail;

    @Column(name = "recruiter_phone", length = 80)
    ...
    public JobApplicationEntity(UUID id, UserAccountEntity owner) {
        this.id = id;
        this.owner = owner;
    }
    ...
    public void update(String company,
        ...
        this.recruiterName = blankToNull(recruiterName);
        this.recruiterEmail = blankToNull(recruiterEmail);
        this.recruiterPhone = blankToNull(recruiterPhone);
    ...
    public void moveTo(RecruitmentStage stage, Instant now) {
        ...
        this.status = stage.impliedStatus();
        this.lastUpdated = now;
        if (stage == RecruitmentStage.OFFRE || stage == RecruitmentStage.CLOTURE) {
            this.responseDate = this.responseDate == null ? LocalDate.now() : this.responseDate;
        }
    }

    public void addInterview(InterviewEntity interview, Instant now) {
    ...
    public BigDecimal getSalaryTarget() { return salaryTarget; }
    ...
    public List<InterviewEntity> getInterviews() { return interviews; }
