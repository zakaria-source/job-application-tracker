...
public class ApplicationService {
    ...
    public ApplicationResponse update(UUID userId, UUID applicationId, ApplicationRequest request) {
        ...
        boolean interviewsChanged = reconcileInterviews(application, request.interviews(), now);

        tracking.recordUpdated(application, now);
        tracking.recordStageChanged(application, previousStage, now);
        if (interviewsChanged) {
    ...
    public ApplicationResponse move(UUID userId, UUID applicationId, RecruitmentStage stage) {
        JobApplicationEntity application = requireOwned(userId, applicationId);
        RecruitmentStage previous = application.getStage();
        Instant now = Instant.now();
    ...
    private JobApplicationEntity createEntity(UUID userId, ApplicationRequest request, boolean recordEvent) {
        ...
        apply(application, request, now);
        reconcileInterviews(application, request.interviews(), now);
        applications.save(application);

        if (recordEvent) {
            tracking.recordCreated(application, now);
            tracking.syncLegacyFollowUp(application, null, now);
    ...
    private static boolean reconcileInterviews(
        ...
        Map<UUID, InterviewEntity> existingById = new HashMap<>();
        for (InterviewEntity interview : application.getInterviews()) {
            existingById.put(interview.getId(), interview);
        }
        ...
        for (InterviewRequest request : requests) {
            InterviewEntity interview = request.id() == null ? null : existingById.get(request.id());
            if (interview == null) {
                interview = new InterviewEntity(UUID.randomUUID(), application);
                application.addInterview(interview, now);
                changed = true;
            } else {
                retainedExistingIds.add(interview.getId());
                changed |= interviewChanged(interview, request);
            }

            interview.update(request.date(), request.type(), request.notes(), request.reminderSet());
        }

        int sizeBeforeRemoval = application.getInterviews().size();
        application.getInterviews().removeIf(interview ->
            existingById.containsKey(interview.getId()) && !retainedExistingIds.contains(interview.getId())
        );
        return changed || application.getInterviews().size() != sizeBeforeRemoval;
    }

    private static boolean interviewChanged(InterviewEntity interview, InterviewRequest request) {
        return !Objects.equals(interview.getDate(), request.date())
            || interview.getType() != request.type()
            || !Objects.equals(interview.getNotes(), request.notes() == null ? "" : request.notes())
            || interview.isReminderSet() != request.reminderSet();
    }
