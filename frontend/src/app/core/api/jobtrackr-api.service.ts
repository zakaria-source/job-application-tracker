...
interface FollowUpDto extends Omit<FollowUp, 'scheduledFor' | 'completedAt' | 'createdAt' | 'updatedAt'> {
  scheduledFor: string; completedAt?: string | null; createdAt: string; updatedAt: string;
}
...
export class JobTrackrApiService {
  ...
  listApplications(): Observable<JobApplication[]> {
    return this.http.get<ApplicationDto[]>(this.applicationsUrl).pipe(map(items => items.map(item => this.hydrateApplication(item))));
  }
  createApplication(application: JobApplication): Observable<JobApplication> {
    return this.http.post<ApplicationDto>(this.applicationsUrl, this.toApplicationRequest(application)).pipe(map(item => this.hydrateApplication(item)));
  }
  updateApplication(application: JobApplication): Observable<JobApplication> {
    return this.http.put<ApplicationDto>(`${this.applicationsUrl}/${encodeURIComponent(application.id)}`, this.toApplicationRequest(application)).pipe(map(item => this.hydrateApplication(item)));
  }
  moveApplication(id: string, stage: RecruitmentStage): Observable<JobApplication> {
    return this.http.patch<ApplicationDto>(`${this.applicationsUrl}/${encodeURIComponent(id)}/stage`, {stage}).pipe(map(item => this.hydrateApplication(item)));
  }
  ...
  scheduleFollowUp(id: string, date: Date): Observable<FollowUp> {
    return this.http.post<FollowUpDto>(`${this.applicationsUrl}/${encodeURIComponent(id)}/follow-ups`, {scheduledFor: this.toLocalDate(date)})
      .pipe(map(item => this.hydrateFollowUp(item)));
  }
  completeCurrentFollowUp(id: string): Observable<FollowUp> {
    return this.http.patch<FollowUpDto>(`${this.applicationsUrl}/${encodeURIComponent(id)}/follow-ups/current/complete`, {})
      .pipe(map(item => this.hydrateFollowUp(item)));
  }
  snoozeFollowUp(applicationId: string, followUpId: string, date: Date): Observable<FollowUp> {
    return this.http.patch<FollowUpDto>(`${this.applicationsUrl}/${encodeURIComponent(applicationId)}/follow-ups/${encodeURIComponent(followUpId)}/snooze`, {scheduledFor: this.toLocalDate(date)})
      .pipe(map(item => this.hydrateFollowUp(item)));
  }
  getInterviewDebriefs(id: string): Observable<InterviewDebrief[]> {
    return this.http.get<InterviewDebriefDto[]>(`${this.applicationsUrl}/${encodeURIComponent(id)}/debriefs`)
      .pipe(map(items => items.map(item => ({...item, updatedAt: new Date(item.updatedAt)}))));
  }
  saveInterviewDebrief(applicationId: string, interviewId: string, input: InterviewDebriefInput): Observable<InterviewDebrief> {
    return this.http.put<InterviewDebriefDto>(`${this.applicationsUrl}/${encodeURIComponent(applicationId)}/interviews/${encodeURIComponent(interviewId)}/debrief`, input)
      .pipe(map(item => ({...item, updatedAt: new Date(item.updatedAt)})));
  }
  ...
  getProfile(): Observable<UserProfile> { return this.http.get<UserProfile>(this.profileUrl); }
  updateProfile(profile: UserProfile): Observable<UserProfile> { return this.http.put<UserProfile>(this.profileUrl, profile); }

  private hydrateApplication(raw: ApplicationDto): JobApplication {
    return {...raw, applicationDate: this.localDate(raw.applicationDate), lastUpdated: new Date(raw.lastUpdated),
  ...
  private hydrateFollowUp(raw: FollowUpDto): FollowUp {
    return {...raw, scheduledFor: this.localDate(raw.scheduledFor), completedAt: raw.completedAt ? new Date(raw.completedAt) : undefined,
  ...
  private toApplicationRequest(application: JobApplication): Record<string, unknown> {
    return {company: application.company, position: application.position, applicationDate: this.toLocalDate(application.applicationDate), status: application.status,
      notes: application.notes, responseDate: application.responseDate ? this.toLocalDate(application.responseDate) : null, offerUrl: application.offerUrl ?? null,
      contractType: application.contractType, salaryTarget: application.salaryTarget ?? null, salaryPeriod: application.salaryPeriod,
      followUpDate: application.followUpDate ? this.toLocalDate(application.followUpDate) : null, recruiterName: application.recruiterName ?? null,
      recruiterEmail: application.recruiterEmail ?? null, recruiterPhone: application.recruiterPhone ?? null, stage: application.stage, priority: application.priority,
      interviews: (application.interviews ?? []).map(interview => ({date: interview.date.toISOString(), type: interview.type, notes: interview.notes, reminderSet: interview.reminderSet}))};
  }
  private localDate(value: string): Date { const [year, month, day] = value.split('-').map(Number); return new Date(year, month - 1, day); }
  private toLocalDate(date: Date): string { const year = date.getFullYear(); const month = String(date.getMonth() + 1).padStart(2, '0'); const day = String(date.getDate()).padStart(2, '0'); return `${year}-${month}-${day}`; }
}
