...
export class ApplicationStore {
  ...
  completeFollowUp(id: string, now = new Date()): void { const application = this.getApplicationById(id); if (!application?.followUpDate) return; const optimistic = {...application, followUpDate: undefined, lastUpdated: now}; this.replaceApplication(optimistic); this.api.completeCurrentFollowUp(id).subscribe({next: () => this.refresh(), error: error => { this.replaceApplication(application); console.error('Unable to complete follow-up', error); }}); }
