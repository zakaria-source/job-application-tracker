...
export class ApplicationDetailsComponent implements OnChanges {
    ...
    get nextActionLabel(): string { if (this.application.status === 'Accepté') return 'Offre acceptée'; if (this.application.status === 'Refusé') return 'Candidature clôturée'; if (this.currentFollowUp?.status === 'OVERDUE') return 'Traiter la relance en retard'; if (this.currentFollowUp?.status === 'DUE' || this.isFollowUpDue()) return 'Effectuer la relance prévue'; const nextInterview = this.nextInterview; if (nextInterview) return `Préparer l’entretien du ${new Intl.DateTimeFormat('fr-FR', {day: '2-digit', month: 'short'}).format(nextInterview.date)}`; if (this.currentFollowUp) return `Relancer le ${new Intl.DateTimeFormat('fr-FR', {day: '2-digit', month: 'short'}).format(this.currentFollowUp.scheduledFor)}`; if (this.nextStage) return `Faire avancer vers « ${this.nextStage} »`; return 'Mettre à jour le contexte'; }
    ...
    get timeline(): TimelineEvent[] { const events: TimelineEvent[] = this.activity.map(event => ({id: event.id, date: event.createdAt, title: event.title, detail: event.details, icon: this.iconForEvent(event.type)})); if (!this.activity.some(event => event.type === 'APPLICATION_CREATED')) events.push({id:'applied', date:this.application.applicationDate, title:'Candidature envoyée', detail:`${this.application.company} · ${this.application.position}`, icon:'outgoing_mail'}); for (const interview of this.application.interviews ?? []) if (interview.date > new Date()) events.push({id:`future-${interview.id}`, date:interview.date, title:`Entretien à venir · ${interview.type}`, detail:interview.notes || 'Rendez-vous planifié', icon:'event', future:true}); return events.sort((a,b) => a.date.getTime() - b.date.getTime()); }
    ...
    saveDebrief(interviewId: string): void { if (!this.api) return; const draft = this.debriefDrafts[interviewId]; if (!draft || this.savingDebriefId) return; this.savingDebriefId = interviewId; this.api.saveInterviewDebrief(this.application.id, interviewId, draft).subscribe({next: saved => { this.debriefs[interviewId] = saved; this.savingDebriefId = ''; this.loadActivityOnly(); }, error: () => { this.savingDebriefId = ''; this.trackingError = 'Impossible d’enregistrer le débrief.'; }}); }
    ...
    private loadTracking(): void { if (!this.api) return; this.trackingLoading = true; this.trackingError = ''; this.api.getApplicationActivity(this.application.id).subscribe({next: items => { this.activity = items; this.trackingLoading = false; }, error: () => { this.trackingLoading = false; this.trackingError = 'Certaines données de suivi sont indisponibles.'; }}); this.api.getFollowUps(this.application.id).subscribe({next: items => this.followUps = items, error: () => undefined}); this.api.getApplicationHealth(this.application.id).subscribe({next: health => this.health = health, error: () => undefined}); this.api.getInterviewDebriefs(this.application.id).subscribe({next: items => { this.debriefs = Object.fromEntries(items.map(item => [item.interviewId, item])); this.ensureDebriefDrafts(); }, error: () => undefined}); }
    private loadActivityOnly(): void { if (!this.api) return; this.api.getApplicationActivity(this.application.id).subscribe({next: items => this.activity = items}); }
    private ensureDebriefDrafts(): void { for (const interview of this.application.interviews ?? []) { const saved = this.debriefs[interview.id]; if (!this.debriefDrafts[interview.id]) this.debriefDrafts[interview.id] = {sentiment:saved?.sentiment ?? 'NEUTRAL', questions:saved?.questions ?? '', strengths:saved?.strengths ?? '', improvements:saved?.improvements ?? '', nextAction:saved?.nextAction ?? ''}; else if (saved) this.debriefDrafts[interview.id] = {sentiment:saved.sentiment, questions:saved.questions, strengths:saved.strengths, improvements:saved.improvements, nextAction:saved.nextAction}; } }
